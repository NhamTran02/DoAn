...
def run_crawler(num_posts: int, fanpage_link: str) -> List[Dict]:
    try:
        ...
        logger.info(f"Kết quả từ crawler stdout: {result.stdout}")
        if result.stderr:
        ...
        raise HTTPException(status_code=500, detail=f"Lỗi khi chạy crawler: stdout: {e.stdout}, stderr: {e.stderr}")
    except Exception as e:
        logger.error(f"Lỗi khi chạy crawler: {e}")
...
async def classify(data: InputText):
    ...
    try:
        ...
        # Phân loại văn bản trước
        truncated_text, result = run_model_inference(data.text)

        # Kiểm tra xem text đã tồn tại trong classified_posts chưa
        ...
        # Nếu không trùng, lưu vào cả hai bảng
...
async def update_classified_post(id: int, data: InputText):
    try:
        ...
        # Chuẩn hóa và phân loại lại văn bản mới
        ...
        # Cập nhật trong classified_posts
        neutral = label == "neutral"
        negative = label == "negative"
        positive = label == "positive"
        with conn_classify.cursor() as cur:
...
async def get_label_stats():
    try:
        ...
        with conn_classify.cursor() as cur:
            # Đếm số bài viết theo từng nhãn
            cur.execute("""
                SELECT 
...
async def get_posts_by_date(label: str = None, start_date: str = None, end_date: str = None):
    try:
        ...
        # Nếu không có start_date, lấy ngày của bài viết đầu tiên
        if not start_date:
            with conn_classify.cursor() as cur:
                ...
                if min_date:
                    ...
                    # Nếu không có bài viết nào, mặc định là ngày hiện tại
