...
function NegativePosts() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      setError("");
      try {
        const response = await axios.get("http://localhost:8000/negative-posts", {
          params: { page, limit }
        });
        ...
        setError("Không thể lấy danh sách bài viết tiêu cực");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="section">
      <h2>Bài Viết Tiêu Cực (Tổng: {totalPosts} bài)</h2>
      {error && (
        <div className="error">
          <h3>Lỗi:</h3>
          <p>{error}</p>
